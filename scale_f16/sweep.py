...
#run_cmd = "cs_python run.py --name out --cmaddr "

# CS-2 run
compile_cmd = "cslc code.csl --arch=wse2 --fabric-dims=757,996 --fabric-offsets=1,1 --verbose -o out "
run_cmd = "cs_python run.py --name out --cmaddr ${CS_IP_ADDR}:9000"
...
with open(test_type + '_f' + str(data_size) + '.csv', mode='w') as csv_file:
  ...
  for stride_size in range(1, stride_size_max+1, 1):
    #for size in range(4864, size_max+1, size_min):
    for size in range(size_min, 4864, size_min):
